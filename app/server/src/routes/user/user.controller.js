...
const UserController = {
  register: async function (req, res) {
    ...
    try {
      ...
      const confirmationToken = await auth.generateToken(
        email,
        jwt_conf_secret,
  ...
  login: async function (req, res) {
    ...
    try {
      ...
      const refresh_token = await auth.generateToken(
        email,
        jwt_ref_secret,
        refresh_jwtExpiry
      );
  ...
  refresh_tokens: async function (req, res) {
    ...
    try {
      ...
      const new_access_token = await auth.generateToken(
        email,
        jwt_ac_secret,
        access_jwtExpiry
      );
      const new_refresh_token = await auth.generateToken(
        email,
        jwt_ref_secret,
        refresh_jwtExpiry
      );
